...
export default {
  ...
  methods: {
    ...
    filteredTickers() {
      ...
      const filteredT = this.tickers.filter((ticker) =>
        ticker.name.toLowerCase().includes(this.filter.toLowerCase())
      );

      this.hasNextPage = filteredT.length > end;

      return filteredT.slice(start, end);
    },
